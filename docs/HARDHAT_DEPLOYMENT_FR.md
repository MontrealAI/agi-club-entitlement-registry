...
L’inspection utilise l’empreinte `runtimeCodeHash` du plan approuvé. Pour un déploiement existant sans ce plan, `EXPECTED_RUNTIME_HASH` doit provenir d’un dossier approuvé indépendant, pas d’un contrat inconnu. L’inspection fixe les blocs finalisé/récent, contrôle code, getters et autorité ENS aux deux états, puis leurs hashes à nouveau. Un changement d’administrateur non finalisé, un code absent à l’état finalisé ou des blocs incohérents sont refusés.

Après réussite, `.local/post-deployment.json` donne les ancres, `address` et `runtimeCodeHash` revus. Remplacez les deux exemples ci-dessous par ces valeurs :
