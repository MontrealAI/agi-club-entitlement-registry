...
**Treat the deployer as compromised.** Never give it ENS operator/token approvals, control of a parent name, Safe ownership/modules or access to root keys. Check those external permissions separately: `isAdmin(deployer) = false` only checks direct registry authority. Someone with the deployer key can steal its ETH, consume the nonce, replace a pending transaction or deploy different code without using this helper. The signed plan constrains the helper; it cannot constrain that attacker. Review the completed deployment from a trusted independent checkout/device and provider before accepting its address. All 18 privileged functions and a counterfeit-constructor attack are exercised locally by `npm run test:deployer`, included in qualification.
...
`npm run inspect:mainnet` needs no key or password. Keep the reviewed `.local/deployment-plan.json` and `.local/deployment-broadcast.json`: the plan supplies the expected creation/runtime hashes, sender, nonce and fee bounds; the checkpoint supplies the exact signed transaction hash. If there is no checkpoint, set `DEPLOYMENT_TRANSACTION_HASH` from an independently reviewed transaction record. A conflicting environment hash is rejected; do not delete a checkpoint to accept a replacement transaction.

For read-only recovery without a plan, set `DEPLOYER_ADDRESS`, `DEPLOYMENT_TRANSACTION_HASH` (unless the checkpoint is retained), `EXPECTED_CREATION_HASH` and `EXPECTED_RUNTIME_HASH`, along with `REGISTRY_ADDRESS` and the current `EXPECTED_ADMIN`. All expected values must come from independent reviewed records, not hashes supplied by an unknown contract or the untrusted deployer. This recovery mode establishes creation and identity; it does not reconstruct missing approval or fee-budget evidence. An expired plan may be inspected historically; inspection never renews it or permits another broadcast.

Inspection rejects any creation that has the wrong sender, nonce, bytecode, value, receipt/address or chain, or is not finalized in the canonical creation block. With a plan, its transaction type and exact gas/fee bounds must match too. It checks runtime, registry getters and canonical ENS authority at pinned finalized and latest blocks, then rechecks those blocks and the creation block. A different constructor can install identical runtime with malicious initial storage, so runtime/getters or an Etherscan verification badge alone are insufficient.

After the inspector starts, it archives a previous report and writes `INCOMPLETE` before its checks. Accept only a successfully completed current command and `.local/post-deployment.json` with **both** `creationVerified: true` and `finalityVerified: true`, status `CREATION_AND_IDENTITY_CHECK_PASS_NOT_LAUNCH_AUTHORIZATION`, the reviewed creation transaction and matching block anchors. An archived report or a failed command is not current evidence, including when Hardhat fails before starting the inspector. Use the successful report's `address` and `runtimeCodeHash` below. This check grants no launch authority.
