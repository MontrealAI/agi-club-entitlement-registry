...
4. **Set claim dates / Régler les dates.** Use `setWindow`. The helper accepts strict UTC in `YYYY-MM-DDTHH:mm:ssZ` format or integer Unix seconds. Etherscan itself receives **seconds**, not milliseconds or a date string. The opening and closing seconds are inclusive; zero removes that bound. If both are set, closing must be greater than opening. These are claim windows, not necessarily event start/end times.
