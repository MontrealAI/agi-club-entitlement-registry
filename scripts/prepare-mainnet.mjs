...
try {
 ...
 const plan={schema:'AGIClubDeploymentPlan/1',chainId:1,contract:PROD,sourceSha256:gate.sourceSha256,creationCodeHash:ethers.keccak256(artifact.bytecode),runtimeCodeHash:gate.qualifiedBytecode.runtimeCodeHash,deployer:deployer.toLowerCase(),admin:admin.toLowerCase(),nonce:String(nonce),predictedAddress:predictedAddress.toLowerCase(),gasLimit:String(gasLimit),maxFeePerGas:String(fee.maxFeePerGas),maxPriorityFeePerGas:String(fee.maxPriorityFeePerGas),maxCostWei:String(maxCostWei),createdAt:Math.floor(Date.now()/1000),expiresAt:Math.floor(Date.now()/1000)+1800,evidenceSha256:gate.evidenceSha256};
