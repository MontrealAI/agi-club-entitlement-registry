/** Creates an UNSIGNED, short-lived exact-bytecode empty-registry plan. Sends no transaction. */
...
try {
 ...
 assert.equal(ethers.keccak256(artifact.bytecode),gate.qualifiedBytecode.creationCodeHash,'Artifact differs from the qualified production build; rerun qualification');
 assert.equal(admin.toLowerCase(),gate.qualifiedBytecode.admin,'Root holder differs from qualified fork; repeat fork and review');
 ...
 const plan={schema:'AGIClubDeploymentPlan/2',chainId:1,contract:PROD,sourceSha256:gate.sourceSha256,creationCodeHash:ethers.keccak256(artifact.bytecode),runtimeCodeHash:gate.qualifiedBytecode.runtimeCodeHash,deployer:deployer.toLowerCase(),admin:admin.toLowerCase(),nonce:String(nonce),predictedAddress:predictedAddress.toLowerCase(),gasLimit:String(gasLimit),maxFeePerGas:String(fee.maxFeePerGas),maxPriorityFeePerGas:String(fee.maxPriorityFeePerGas),maxCostWei:String(maxCostWei),createdAt:Math.floor(Date.now()/1000),expiresAt:Math.floor(Date.now()/1000)+1800,evidenceSha256:gate.evidenceSha256};
 validatePlan(plan);save('.local/deployment-plan.json',plan);
 console.log('UNSIGNED EMPTY REGISTRY PLAN — no transaction sent');console.log(JSON.stringify(plan,null,2));console.log('Review reports and plan, then sign through dist/site/deployment.html on the trusted origin.');
