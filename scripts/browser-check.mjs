...
try{
 ...
 for(const page of ['index','member','admin','verify','privacy','legal','deployment','etherscan']){
  ...
  report.checks.push({page,scope:'Keyboard entry, main landmark and control labels',status:'PASS'});
  ...
  for(const width of [1440,768,390,320]){
   ...
   assert(await evaluate('document.documentElement.scrollWidth <= innerWidth+2'),'Horizontal overflow: '+page+' '+width);report.checks.push({page,width,scope:'Responsive reflow',status:'PASS'});
   if(width===1440||width===390){
    ...
    report.checks.push({page,width,scope:'Automated axe checks',status:violations.length?'FAIL':'PASS',violations,manualReview:a11y.incomplete.map(v=>({id:v.id,targets:v.nodes.map(n=>n.target)}))});
