...
// The --upload gate below still rejects it in a public release extraction.
const exclude=new Set(['.git','node_modules','dist','artifacts','cache','qualification','.local','operations','__pycache__']);const files=[];
function walk(dir){for(const e of fs.readdirSync(dir,{withFileTypes:true})){const p=path.join(dir,e.name),rel=p.replaceAll('\\','/').replace(/^\.\//,'');if(exclude.has(e.name)||rel==='frontend/vendor'||rel==='.env')continue;assert(!e.isSymbolicLink(),'No release symlinks');if(e.isDirectory())walk(p);else if(!e.name.endsWith('.pyc'))files.push(rel);}}walk('.');
