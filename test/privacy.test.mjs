...
test('Member page has no submit form and suppresses field autocomplete',()=>{const s=read('frontend/member.html');assert(!/<form\b/i.test(s));for(const id of ['ticketName','ticketEmail','requestPreview']){const tag=s.match(new RegExp('<(?:input|textarea)[^>]*id="'+id+'"[^>]*>'))?.[0];assert(tag);assert(tag.includes('autocomplete="off"'));assert(tag.includes('spellcheck="false"'));assert(!/\sname=/.test(tag));}});
