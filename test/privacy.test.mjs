...
for(const file of ['member.js','private-memory.mjs','verify.js','app.js','member-catalog.mjs','shared/request-email.mjs','language.mjs','language-core.mjs']){
...
test('User acknowledgement precedes clipboard handoff and success clears page',()=>{const s=read('frontend/member.js');assert(s.includes("if(!$('copyConsent').checked)throw Error('COPY_CONSENT')"));assert(s.includes('navigator.clipboard.writeText(formatRequestEmail(packet))'));assert(s.includes("clearPrivate();message(t('Copiée"));});
