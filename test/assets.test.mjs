...
import {validOrigin,validatePolicy,REGISTRY_VERSION} from '../shared/entitlement-request.mjs';
...
test('Browser verifier source byte-identical to canonical source',()=>{for(const f of ['entitlement-request.mjs','ethers-adapter.mjs','request-email.mjs'])assert.equal(fs.readFileSync('shared/'+f,'utf8'),fs.readFileSync('frontend/shared/'+f,'utf8'));});
test('Member, operator and CLI use the same canonical protocol and production adapter',()=>{for(const f of ['frontend/member.js','frontend/verify.js','tools/verify_entitlement_request.mjs']){const src=fs.readFileSync(f,'utf8');assert.match(src,/import[^\n]*verifyEntitlementRequest[^\n]*entitlement-request\.mjs/);assert.match(src,/import[^\n]*createEthersIO[^\n]*ethers-adapter\.mjs/);}});
...
test('Public build copies only named shared/vendor assets, never whole directories',()=>{const s=fs.readFileSync('scripts/package.mjs','utf8');assert(!s.includes('cpSync'));assert(s.includes("['entitlement-request.mjs','ethers-adapter.mjs','deployment-policy.mjs','request-email.mjs']"));assert(s.includes("['ethers.umd.min.js','ETHERS_LICENSE.md']"));});
