...
test('Browser verifier source byte-identical to canonical source',()=>{for(const f of ['ticket-request.mjs','ethers-adapter.mjs'])assert.equal(fs.readFileSync('shared/'+f,'utf8'),fs.readFileSync('frontend/shared/'+f,'utf8'));});
...
test('Public build copies only named shared/vendor assets, never whole directories',()=>{const s=fs.readFileSync('scripts/package.mjs','utf8');assert(!s.includes('cpSync'));assert(s.includes("['ticket-request.mjs','ethers-adapter.mjs','deployment-policy.mjs']"));assert(s.includes("['ethers.umd.min.js','ETHERS_LICENSE.md']"));});
...
function configuredPolicy(source) {
 ...
 validatePolicy({origin:config.expectedOrigin,chainId:config.chainId,registry:config.registryAddress,registryCodeHash:config.registryCodeHash,version:REGISTRY_VERSION,entitlements:config.allowedEntitlements.map(x=>x.startsWith('0x')?x:id(x))});
 return config;
}
...
test('Configurator normalizes hexadecimal entitlement IDs without changing canonical keys',t=>{
 ...
 assert.equal(config.defaultEntitlement,ids[0].toLowerCase());
});
