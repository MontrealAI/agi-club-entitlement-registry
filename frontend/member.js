...
function touch(){clearTimeout(timer);timer=setTimeout(()=>{clearPrivate();message(t('Les coordonnées ont été effacées après 10 minutes d’inactivité. Aucun billet n’a été envoyé par cette page.'));},600000);}
...
async function inspectMember(){
  ...
  $('claim').disabled=Number(r[0])!==0;lockContact(claimed);status(claimed?t('Le claim existe déjà. Préparez seulement la demande de billet.'):t('Vérification terminée.'));
}
...
async function prepare(){
  ...
  status(t('Prête à copier. Ce reçu contient vos coordonnées en clair : gardez-le privé.'));touch();
}
async function copy(){
...
on('demo',()=>{disconnect();demo=true;$('verify').disabled=false;$('authority').textContent=t('Démonstration');$('modeNotice').textContent=t('DÉMONSTRATION — sans événement, sans wallet, sans transaction et sans envoi. Utilisez des données fictives.');$('memberLabel').value='exemple';status(t('Cliquez sur Vérifier pour explorer la confidentialité avec un droit fictif. Aucun événement n’est créé.'));});
