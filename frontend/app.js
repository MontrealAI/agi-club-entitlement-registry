...
function integer(id){const x=$(id).value.trim();if(!/^\d+$/.test(x)||BigInt(x)>1000000n)throw Error('Quota : entier entre 0 et 1 000 000.');return BigInt(x);}
function time(id){let t=$(id).value.trim();if(!t)return 0n;if(!/^\d{4}-\d\d-\d\dT\d\d:\d\d(?::\d\d)?Z$/.test(t))throw Error('Date UTC requise, par exemple 2026-09-22T23:00:00Z.');let n=Date.parse(t);if(!Number.isFinite(n))throw Error('Date non valide.');return BigInt(Math.floor(n/1000));}
function dates(){let a=time('opens'),b=time('closes');if(a&&b&&b<=a)throw Error('La fermeture doit suivre l’ouverture.');return[a,b];}
function iso(n){return Number(n)?new Date(Number(n)*1000).toISOString().replace('.000',''):'';}
    ...
    try{
      ...
      for(let offset=start;offset<target;offset+=50){
        ...
        for(const id of ids){
          ...
          if(!isCurrent())return;
          next.push({id,category:e[0],hash:e[1],cap:Number(e[2]),active:Number(e[3]),unique:Number(e[4]),opens:e[5],closes:e[6],state:Number(e[7]),fr,en,uri});
        }
      }
...
bind('duplicate',()=>{let n=prompt('Identifiant du nouvel avantage (ex. IA101_2026_11_17)');if(n)return preview('duplicateEntitlement',[idRequired(),key(n)],'Créer une copie en brouillon. Aucune réclamation, date ou métadonnée ne sera reprise.');});
